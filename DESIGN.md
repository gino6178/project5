...
**（二）圖定義「什麼算違規」，而且是學來的。** 室內場景的合理佈置本來就有貼靠——餐椅塞在桌下、床頭櫃靠著床。用一個與類別無關的重疊懲罰會懲罰掉我們正要保留的結構：真實 3D-FRONT 房間在這種指標下就有 0.388 的碰撞率，而照它訓練會把佈局推離真值（實測 val_recon 上升 8 倍）。

手寫一張「哪些關係允許貼靠」的規則表只是把建模搬進規則裡。因此我們讓模型**學習每一對物件該有的間距**：`PairGapPredictor` 從邊特徵與兩個物件的條件預測期望間距與容忍度，並由**真實佈局中觀察到的間距**監督，所以不會塌縮成「什麼都允許」；評分佈局時預測值被 detach，逼佈局向學到的間距靠攏，而不是讓預測去遷就佈局。驗證：模型自行學到餐桌–餐椅（有邊）期望間距 **−0.37 m**（該重疊），桌–無關物件（無邊）**+1.75 m**（該分開）。

**（三）圖也負責傳遞資訊。** 邊特徵同時調變注意力 logits 與訊息（graph transformer，而非只加一個純量偏置）。

**（四）確定性、單次前向。** 沒有噪聲、沒有 ODE、沒有 $k$ 個候選。從仿射轉置出發做 $L$ 次殘差精修，一次前向得到答案。
...
- **間距監督**：訓練 `PairGapPredictor`，讓「正確的間距」由語料決定
- **可走性**：可微光柵化的 blocked-walkway 懲罰（`walkable.py`）。PhyScene 在可走比上勝過 project4（0.963 對 0.900），這是唯一輸的軸，直接優化它
